...
//		ResponseEntity.ok().body(tourDetails);
		if (tourDetails.getId() == 0) {
	...
	public ResponseEntity<String> deleteTourDetail(int tripID) {
		...
		if (tourDetails.getId() == 0) {
			...
			return new ResponseEntity<String>("Error: Tour Details Not Found for this ID: " + tripID, HttpStatus.NOT_FOUND);
		} else {
